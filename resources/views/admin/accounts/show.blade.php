...
@section('title', 'Account Details - ' . $account->account_title)
...
@section('content')
    <div class="space-y-6">
            ...
            <ol class="inline-flex items-center space-x-1 md:space-x-3">
                ...
                <li>
                    <div class="flex items-center">
                        ...
                        <a href="{{ route('admin.accounts.index') }}" class="ml-1 text-sm font-medium text-[#706f6c] hover:text-[#1b1b18] dark:text-[#A1A09A] dark:hover:text-[#EDEDEC] md:ml-2">Accounts</a>
                    </div>
                </li>
                <li aria-current="page">
                    <div class="flex items-center">
                        ...
                        <span class="ml-1 text-sm font-medium text-[#1b1b18] dark:text-[#EDEDEC] md:ml-2">{{ $account->account_title }}</span>
                    </div>
                </li>
            </ol>
        </nav>
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                <h1 class="text-3xl font-bold text-[#1b1b18] dark:text-[#EDEDEC]">{{ $account->account_title }}</h1>
                <div class="flex items-center space-x-3 mt-2">
                    <p class="text-[#706f6c] dark:text-[#A1A09A]">{{ $account->branch->name }}</p>
                    <span class="text-[#706f6c] dark:text-[#A1A09A]">•</span>
                    <p class="text-[#706f6c] dark:text-[#A1A09A]">ID: {{ Str::limit($account->id, 8) }}</p>
                    <span class="text-[#706f6c] dark:text-[#A1A09A]">•</span>
                    <p class="text-[#706f6c] dark:text-[#A1A09A]">Created {{ $account->created_at->format('M j, Y') }}</p>
                </div>
            </div>
                    ...
                    <div class="col-span-10">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                            <div class="flex items-center space-x-2">
                                ...
                                <span class="text-[#706f6c] dark:text-[#A1A09A]">{{ $account->branch->name }}</span>
                            </div>
                            <div class="flex items-center space-x-2">
                                <svg class="w-4 h-4 text-[#706f6c] dark:text-[#A1A09A]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4"></path>
                                </svg>
                                <span class="text-[#706f6c] dark:text-[#A1A09A]">{{ $account->collaterals->count() }} collaterals</span>
                            </div>
                            @if($account->collaterals->count() > 0)
                                <div class="flex items-center space-x-2">
                                    ...
                                    <span class="text-[#706f6c] dark:text-[#A1A09A]">RM {{ number_format($account->collaterals->sum('estimated_value_rm'), 2) }}</span>
                                </div>
                            @endif
                        </div>
                    </div>
                ...
                <div class="px-6 py-4 bg-gray-50 dark:bg-[#1a1a19] border-t border-[#e3e3e0] dark:border-[#3E3E3A]">
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <p class="text-sm text-[#706f6c] dark:text-[#A1A09A]">This account is waiting for approval to become active.</p>
                        </div>
                        <div class="flex space-x-3">
                            <form method="POST" action="{{ route('admin.accounts.approve', $account) }}" class="inline">
                                ...
                                <button type="submit"
                                        ...
                                        onclick="return confirm('Approve account {{ $account->account_title }}?')">
                                    <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            <form method="POST" action="{{ route('admin.accounts.reject', $account) }}" class="inline">
                                ...
                                <button type="submit"
                                        ...
                                        onclick="return confirm('Reject account {{ $account->account_title }}?')">
                                    <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div class="bg-white dark:bg-[#161615] rounded-xl border border-[#e3e3e0] dark:border-[#3E3E3A] overflow-hidden">
                ...
                <div class="p-6">
                    <div class="space-y-6">
                        <div>
                            <label class="block text-sm font-medium text-[#706f6c] dark:text-[#A1A09A] mb-2">Account Title</label>
                            <div class="flex items-center space-x-2">
                                <p class="text-[#1b1b18] dark:text-[#EDEDEC] font-medium">{{ $account->account_title }}</p>
                            </div>
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-[#706f6c] dark:text-[#A1A09A] mb-2">Branch Location</label>
                            <div class="flex items-center space-x-3">
                                <p class="text-[#1b1b18] dark:text-[#EDEDEC]">{{ $account->branch->name }}</p>
                            </div>
                            @if($account->branch->branchAddress)
                                <p class="text-sm text-[#706f6c] dark:text-[#A1A09A] mt-1">{{ $account->branch->branchAddress->full_address }}</p>
                            @endif
                        </div>
                        ...
                        @if($account->description)
                            <div>
                                ...
                                <p class="text-[#1b1b18] dark:text-[#EDEDEC]">{{ $account->description }}</p>
                            </div>
                        @endif

                        <div>
                            <label class="block text-sm font-medium text-[#706f6c] dark:text-[#A1A09A] mb-2">Account ID</label>
                            <div class="flex items-center space-x-2">
                                <p class="text-[#1b1b18] dark:text-[#EDEDEC] font-mono text-sm bg-gray-100 dark:bg-gray-800 px-3 py-1 rounded">{{ $account->id }}</p>
                                <button onclick="copyToClipboard('{{ $account->id }}')" class="text-[#706f6c] hover:text-[#1b1b18] dark:text-[#A1A09A] dark:hover:text-[#EDEDEC]">
            ...
            <div class="bg-white dark:bg-[#161615] rounded-xl border border-[#e3e3e0] dark:border-[#3E3E3A] overflow-hidden">
                ...
                <div class="p-6">
                    <div class="space-y-6">
                        ...
                        <!-- Collateral Summary -->
                        <div>
                            <label class="block text-sm font-medium text-[#706f6c] dark:text-[#A1A09A] mb-3">Collateral Summary</label>
                            <div class="grid grid-cols-2 gap-4">
                                <div class="flex items-center justify-between p-3 bg-gray-50 dark:bg-[#1a1a19] rounded-lg">
                                    <div class="flex items-center space-x-2">
                                        ...
                                        <span class="text-sm text-[#1b1b18] dark:text-[#EDEDEC]">Total Items</span>
                                    </div>
                                ...
                                <div class="flex items-center justify-between p-3 bg-gray-50 dark:bg-[#1a1a19] rounded-lg">
                                    ...
                                    <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-green-100 dark:bg-green-900/20 text-green-800 dark:text-green-200">
                                        RM {{ number_format($account->collaterals->sum('estimated_value_rm'), 0) }}
                                    </span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="flex items-center justify-between py-3 border-b border-[#e3e3e0] dark:border-[#3E3E3A] last:border-b-0">
                            <div class="flex items-center space-x-3">
                                ...
                                <div>
                                    ...
                                    <p class="text-xs text-[#706f6c] dark:text-[#A1A09A]">Account registration completed</p>
                                </div>
                            </div>
                            <div class="text-right">
                                <p class="text-sm text-[#1b1b18] dark:text-[#EDEDEC]">{{ $account->created_at->format('M j, Y') }}</p>
                                <p class="text-xs text-[#706f6c] dark:text-[#A1A09A]">{{ $account->created_at->format('g:i A') }}</p>
                            </div>
                        </div>
                        ...
                        <div class="flex items-center justify-between py-3">
                            <div class="flex items-center space-x-3">
                                ...
                                <div>
                                    ...
                                    <p class="text-xs text-[#706f6c] dark:text-[#A1A09A]">Account modification</p>
                                </div>
                            </div>
                            <div class="text-right">
                                <p class="text-sm text-[#1b1b18] dark:text-[#EDEDEC]">{{ $account->updated_at->format('M j, Y') }}</p>
                                <p class="text-xs text-[#706f6c] dark:text-[#A1A09A]">{{ $account->updated_at->format('g:i A') }}</p>
                            </div>
                        </div>
                    </div>
                </div>
                            ...
                            <div class="flex items-center justify-between p-4 bg-blue-50 dark:bg-blue-900/10 rounded-lg border border-blue-200 dark:border-blue-800">
                                <div class="flex items-center space-x-3">
                                    ...
                                    <div>
                                        ...
                                        <p class="text-sm text-[#706f6c] dark:text-[#A1A09A]">{{ $account->creator->full_name }} (@{{ $account->creator->username }})</p>
                                    </div>
                                </div>
                                <div class="text-right">
                                    <p class="text-xs text-[#706f6c] dark:text-[#A1A09A]">{{ $account->created_at->format('M j, Y') }}</p>
                                </div>
                            </div>
                        @endif
                            ...
                            <div class="flex items-center justify-between p-4 bg-green-50 dark:bg-green-900/10 rounded-lg border border-green-200 dark:border-green-800">
                                <div class="flex items-center space-x-3">
                                    ...
                                    <div>
                                        ...
                                        <p class="text-sm text-[#706f6c] dark:text-[#A1A09A]">{{ $account->approvedBy->full_name }} (@{{ $account->approvedBy->username }})</p>
                                    </div>
                                </div>
                                <div class="text-right">
            ...
            <div class="bg-white dark:bg-[#161615] rounded-xl border border-[#e3e3e0] dark:border-[#3E3E3A] overflow-hidden">
                <div class="px-6 py-4 bg-gray-50 dark:bg-[#1a1a19] border-b border-[#e3e3e0] dark:border-[#3E3E3A]">
                    <div class="flex items-center space-x-2">
                        ...
                        <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-blue-100 dark:bg-blue-900/20 text-blue-800 dark:text-blue-200">
                            {{ $account->collaterals->count() }} {{ Str::plural('item', $account->collaterals->count()) }}
                        </span>
                    </div>
                </div>
                ...
                <div class="p-6">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        @foreach($account->collaterals as $collateral)
                            <div class="border border-[#e3e3e0] dark:border-[#3E3E3A] rounded-lg p-4 hover:bg-gray-50 dark:hover:bg-[#1a1a19] transition-colors">
                                <div class="flex items-center justify-between mb-3">
                                    <h4 class="font-medium text-[#1b1b18] dark:text-[#EDEDEC] flex items-center space-x-2">
                                        ...
                                        <span>{{ $collateral->item_name }}</span>
                                    </h4>
                                ...
                                <div class="text-sm text-[#706f6c] dark:text-[#A1A09A] space-y-1">
                                    <p class="font-medium text-[#1b1b18] dark:text-[#EDEDEC]">{{ $collateral->category }}</p>
                                    <p class="text-brand font-semibold">RM {{ number_format($collateral->estimated_value_rm, 2) }}</p>
